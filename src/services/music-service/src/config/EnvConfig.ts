...
class EnvConfig {
    ...
    public getRpcLoveServicePort(): number {
        const port = this.env.GRPC_PORT_LOVE_SERVICE;
        if (!port) {
            throw new Error("GRPC_PORT_LOVE_SERVICE is not defined in the environment variables.");
        }
        return parseInt(port, 10);
    }

    public getRpcHistoryServicePort(): number {
        const port = this.env.GRPC_PORT_HISTORY_SERVICE;
        if (!port) {
            throw new Error("GRPC_PORT_HISTORY_SERVICE is not defined in the environment variables.");
        }
        return parseInt(port, 10);
    }
