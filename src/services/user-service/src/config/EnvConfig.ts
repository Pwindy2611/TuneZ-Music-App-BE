...
class EnvConfig {
    ...
    public getRpcPlaylistServicePort(): number {
        const port = this.env.GRPC_PORT_PLAYLIST_SERVICE;
        if (!port) {
            throw new Error("GRPC_PORT_PLAYLIST_SERVICE is not defined in the environment variables.");
        }
        return parseInt(port, 10);
    }
    ...
    public getRpcFollowServicePort(): number {
        const port = this.env.GRPC_PORT_FOLLOW_SERVICE;
        if (!port) {
            throw new Error("GRPC_PORT_FOLLOW_SERVICE is not defined in the environment variables.");
        }
        return parseInt(port, 10);
    }
